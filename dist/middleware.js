...
function routeWizard(options = {}) {
    const { controllersPath = (0, path_1.join)(process.cwd(), 'controllers'), methodMappings, separator = '-', ignorePatterns = [], logEnabled = true, routePrefix = '', enableCache = true, cacheTtl = 30000, // 30 seconds default
    enableWatch = false, watchOptions = {} } = options;
    if (!separator || separator.length === 0) {
        throw new Error('Separator cannot be empty. Use a non-empty string like "-" or "_".');
    }
    // Create logger function
    const log = (0, utils_js_1.createLogger)(logEnabled);
    // Cache for routes
    let routeCache = null;
    ...
    const startWatching = () => {
        ...
        try {
            ...
            (0, fs_1.watch)(watchPath, options, (eventType, filename) => {
                if (filename) {
